...
export function buildFrost(
  ...
  const e = 60
  // brightness/saturation match the baked mosaic veil (bake.ts) so the GM
  // preview tone predicts what players actually get
  ctx.filter = 'blur(28px) brightness(0.4) saturate(0.8)'
